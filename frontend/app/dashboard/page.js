...
export default function Dashboard() {
  ...
  const handleVoiceInput = () => {
    ...
    recognition.onresult = async (event) => {
      ...
      try {
        ...
        const { description, amount, date, type, category_suggestion } =
          res.data;
        ...
        const matchedCat = categories.find(
          (c) => c.name.toLowerCase() === category_suggestion?.toLowerCase(),
        );
  ...
  const fetchSummary = useCallback(async () => {
    try {
      ...
      if (sRes.data) {
        setSummary({
          balance: sRes.data.balance ?? 0,
          totalIncome: sRes.data.totalIncome ?? 0,
          totalExpense: sRes.data.totalExpense ?? 0,
        });
      }
        ...
        <main className="flex-1 p-4 sm:p-6 md:p-10 lg:p-12 w-full mx-auto max-w-[1400px]">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
            <div className="space-y-1">
              <div className="flex items-center gap-2 mb-2 px-1">
                ...
                <span className="text-[10px] font-black text-indigo-400 uppercase tracking-[4px]">
              ...
              <p className="text-slate-400 font-medium text-sm">
                Managing {transactions.length || 0} transactions in current
                cycle.
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              >
                <MicrophoneIcon
                  className={`w-5 h-5 ${isListening ? "text-indigo-400" : "text-slate-400"}`}
                />
              ...
              >
                <div className="bg-white/[0.03] backdrop-blur-2xl border border-white/10 rounded-[2rem] p-8 sm:p-10 shadow-2xl relative">
                  ...
                  <h3 className="text-xl font-bold text-white mb-8 flex items-center gap-3">
                    <div className="w-1.5 h-6 bg-indigo-500 rounded-full" /> New
                    Transaction
                  </h3>
                  <form
                  ...
                  >
                    <div className="md:col-span-1 space-y-2">
                      <label className="text-[11px] font-black text-white uppercase tracking-widest px-1">
                        Type
                      </label>
                      <div className="flex p-1.5 bg-white/5 rounded-2xl border border-white/5">
                        <button
                          ...
                          className={`flex-1 py-3 rounded-xl text-[11px] font-bold uppercase transition-all ${form.type === "income" ? "bg-white/10 text-emerald-400 shadow-lg" : "text-white"}`}
                        >
                          Income
                        </button>
                        <button
                          ...
                          className={`flex-1 py-3 rounded-xl text-[11px] font-bold uppercase transition-all ${form.type === "expense" ? "bg-white/10 text-rose-400 shadow-lg" : "text-white"}`}
                        >
                          Expense
                        </button>
                      </div>
                    ...
                    <div className="md:col-span-1 space-y-2">
                      ...
                      <input
                        ...
                        className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-6 text-white text-xl font-bold placeholder:text-slate-700 outline-none focus:border-indigo-500/50 transition-all"
                        required
                      />
                    </div>
                    <div className="md:col-span-1 space-y-2">
                      ...
                      <input
                        ...
                        className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-6 text-white font-medium placeholder:text-slate-700 outline-none focus:border-indigo-500/50 transition-all"
                        required
                      />
                    </div>
                    <div className="md:col-span-1 space-y-2">
                      ...
                      <select
                        ...
                        className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-6 text-white font-medium placeholder:text-slate-700 outline-none focus:border-indigo-500/50 appearance-none bg-no-repeat bg-[right_1.5rem_center] transition-all cursor-pointer"
                        required
                      >
                    ...
                    <div className="md:col-span-4 mt-2">
                      <button
                        type="submit"
                        className="w-full py-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl font-bold transition-all shadow-xl shadow-indigo-600/10"
                      >
                        Confirm Transaction
                      </button>
                    </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            <motion.div
              ...
              className="bg-white/[0.03] backdrop-blur-2xl border border-white/10 rounded-[2.5rem] p-8 flex flex-col gap-6 group hover:border-white/20 transition-all"
            >
              <div className="flex justify-between items-start">
                ...
                <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest bg-emerald-400/10 px-3 py-1 rounded-full border border-emerald-400/20">
                  Operational
                </span>
              </div>
              <div>
                <p className="text-[11px] font-bold text-white uppercase tracking-[3px] mb-2 italic">
                  Total Balance
                </p>
            ...
            <motion.div
              ...
              className="bg-indigo-600/10 backdrop-blur-2xl border border-indigo-500/20 rounded-[2.5rem] p-8 flex flex-col gap-6 group hover:border-indigo-500/40 transition-all"
            >
              <div className="flex justify-between items-start">
                ...
                <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest bg-indigo-400/10 px-3 py-1 rounded-full border border-indigo-400/20">
                  Income
                </span>
              </div>
              <div>
            ...
            <motion.div
              ...
              className="bg-rose-600/5 backdrop-blur-2xl border border-rose-500/10 rounded-[2.5rem] p-8 flex flex-col gap-6 group hover:border-rose-500/30 transition-all"
            >
              <div className="flex justify-between items-start">
                ...
                <span className="text-[10px] font-bold text-rose-400 uppercase tracking-widest bg-rose-400/10 px-3 py-1 rounded-full border border-rose-400/20">
                  Spending
                </span>
              </div>
              <div>
          ...
          <div className="grid lg:grid-cols-12 gap-8 lg:gap-10">
            <div className="lg:col-span-8 flex flex-col gap-10">
              ...
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="bg-white/[0.03] backdrop-blur-2xl border border-white/10 rounded-[2.5rem] p-8 sm:p-10 relative overflow-hidden group"
              >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-12 relative z-10">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs font-medium text-white">
                      Analytics visualization across recordsets
                    </p>
                  </div>
                  <div className="flex items-center gap-2 bg-white/5 p-1 rounded-2xl border border-white/5">
                    <button
                      onClick={() => setChartView("live")}
                      className={`px-5 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest italic transition-all ${chartView === "live" ? "bg-indigo-600 text-white shadow-xl" : "text-slate-400 hover:text-white"}`}
                    >
                      Live
                    </button>
                    <button
                      onClick={() => setChartView("history")}
                      className={`px-5 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest italic transition-all ${chartView === "history" ? "bg-indigo-600 text-white shadow-xl" : "text-slate-400 hover:text-white"}`}
                    >
                      History
                    </button>
                  </div>
              ...
              <div className="space-y-8">
                <div className="flex items-center justify-between px-2">
                  ...
                  <div className="bg-white/5 border border-white/10 rounded-2xl flex items-center px-4 overflow-hidden focus-within:border-indigo-500/50 transition-all min-w-[250px]">
                    ...
                    <input
                      type="text"
                      placeholder="Search..."
                      className="w-full h-11 bg-transparent pl-3 text-xs font-medium text-white outline-none placeholder:text-white"
                      onChange={(e) =>
                ...
                <div className="flex justify-between items-center py-6 px-10 bg-white/[0.03] backdrop-blur-2xl border border-white/10 rounded-[2.5rem]">
                  <div className="space-y-1">
                    <p className="text-[10px] font-bold text-white uppercase tracking-[2px]">
                      Page <span className="text-white">{page}</span> of{" "}
                      {totalPages}
                    </p>
                    <div className="h-1 bg-white/5 rounded-full w-20 overflow-hidden">
                      <div
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      disabled={page === 1}
                      className="w-11 h-11 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 hover:text-white transition-all disabled:opacity-20 flex items-center justify-center active:scale-90"
                    >
            ...
            <div className="lg:col-span-4 flex flex-col gap-8">
                  ...
                  <div className="space-y-8">
                    <div className="space-y-4">
                      <div className="flex justify-between items-end border-b border-white/5 pb-4">
                        <span className="text-xs font-bold text-white uppercase tracking-widest italic leading-none">
                          Net Balance
                        </span>
                      ...
                      <div className="flex justify-between items-end border-b border-white/5 pb-4">
                        ...
                        <span className="text-2xl font-black italic text-rose-400 leading-none">
                          {(
              ...
              <div className="bg-white/[0.03] backdrop-blur-2xl border border-white/10 rounded-[2.5rem] p-10">
                <h3 className="text-lg font-bold text-white uppercase tracking-tight mb-8">
                  Categories
                </h3>
                  ...
                  {categories.slice(0, 4).map((cat, i) => (
                    <div key={cat._id} className="group cursor-pointer">
                      ...
                      <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                        <div
                          className="h-full rounded-full transition-all duration-1000 shadow-[0_0_10px_rgba(0,0,0,0.3)] shadow-[cat.color]"
                          style={{
