...
const FEATURES = [
  { icon: "💳", title: "Smart Tracking",   desc: "Log income & expenses easily with categories and simple labels.",              color: "from-teal-500 to-emerald-500" },
  { icon: "📊", title: "Visual Charts",    desc: "See your spending habits with clear pie and bar charts.",                     color: "from-violet-500 to-purple-500" },
  { icon: "🎯", title: "Budgeting",        desc: "Set monthly limits and see how much you have left to spend.",                 color: "from-blue-500 to-cyan-500" },
  { icon: "🏆", title: "Savings Goals",    desc: "Save for what matters most with progress bars to keep you motivated.",        color: "from-amber-500 to-orange-500" },
  { icon: "🔁", title: "Recurring Bills",  desc: "Never miss a payment. Track rent, subscriptions, and more.",                 color: "from-rose-500 to-pink-500" },
  { icon: "🔒", title: "Secure & Private", desc: "Your financial data is protected and kept private at all times.",            color: "from-slate-500 to-slate-700" },
];

const STEPS = [
  { num: "01", icon: "👤", title: "Create Your Account",       desc: "Sign up securely with email verification and get access instantly." },
  { num: "02", icon: "⚙️", title: "Personalize Categories",    desc: "Create custom categories like Food, Travel, Bills for better tracking." },
  { num: "03", icon: "💬", title: "Add Transactions Smartly",  desc: "Speak like 'I spent ₹500 on food' and AI will auto-detect details." },
  { num: "04", icon: "🎤", title: "Voice Input",               desc: "Speak your expenses and TrackFin converts them into transactions." },
  { num: "05", icon: "🤖", title: "Chat with AI",              desc: "Ask insights like 'Where did I spend most this month?'" },
  { num: "06", icon: "📊", title: "Track & Analyze",           desc: "View charts, reports, and insights to control your finances." },
];

const SCREENSHOTS = [
  { src: "/hero-dashboard-v2.png", title: "All-in-One Dashboard", desc: "Your complete financial overview — balance, income, expenses, and savings rate.", badge: "Dashboard" },
...
const REVIEWS = [
  { name: "Priya Sharma", role: "Freelancer", text: "TrackFin completely changed how I manage my income. The budget tracking is a total game-changer!", avatar: "P", color: "from-teal-500 to-emerald-600" },
  { name: "Arjun Mehta",  role: "Engineer",   text: "The recurring transactions feature alone is worth it. My subscriptions never sneak up on me anymore.", avatar: "A", color: "from-violet-500 to-purple-600" },
];
...
<InstallPrompt />
      ...
      <nav className={`fixed left-0 right-0 z-50 transition-all duration-300 top-0 ${
        scrolled ? "bg-[#060b18]/90 backdrop-blur-xl border-b border-white/[0.06] shadow-xl" : ""
      }`}>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              ...
              title={
                ...
                : notifPerm === "denied"  ? "Notifications blocked — enable in settings"
                : "Enable notifications"
              }
              className={`w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center transition-all border text-sm ${
                notifPerm === "granted" ? "bg-teal-500/20 border-teal-500/50 text-teal-400"
                : notifPerm === "denied"  ? "bg-red-500/10 border-red-500/30 text-red-400"
      ...
      >
        ...
        <motion.div style={{ y: heroY, opacity: heroOpacity }} className="relative z-10 w-full flex flex-col items-center">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold mb-4 sm:mb-6 tracking-tight leading-tight">
            Master Your{" "}
            <span className="text-teal-400 font-black italic">Financial Life</span>
          </h1>
          <p className="max-w-xl sm:max-w-2xl text-base sm:text-lg text-slate-400 mb-8 sm:mb-10 mx-auto leading-relaxed px-2">
            The all-in-one personal finance platform — track your spending, set budgets, and achieve your goals with ease.
          </p>
        ...
        >
          <div className="relative rounded-xl sm:rounded-2xl overflow-hidden border border-white/10 shadow-3xl bg-slate-900">
            <Image src="/hero-dashboard-v2.png" alt="TrackFin Dashboard" width={1200} height={750} className="w-full h-auto mt-6 sm:mt-8 opacity-90" priority />
      ...
      <section id="features" className="py-20 sm:py-24 px-4 sm:px-6 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-8">
          {FEATURES.map((f) => (
            <div key={f.title} className="p-6 sm:p-8 rounded-3xl bg-white/[0.02] border border-white/5 hover:border-teal-500/50 transition group">
              <div className={`w-11 h-11 sm:w-12 sm:h-12 rounded-2xl bg-gradient-to-br ${f.color} flex items-center justify-center text-lg sm:text-xl mb-5 sm:mb-6 shadow-xl group-hover:scale-110 transition`}>
                {f.icon}
              </div>
      ...
      <section id="how-it-works" className="py-20 sm:py-24 px-4 sm:px-6 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-8">
          {STEPS.map((step) => (
            <div key={step.num} className="p-6 sm:p-8 rounded-3xl bg-white/[0.02] border border-white/5 hover:border-teal-500/40 transition group">
      ...
      {/* ── PREVIEW ──────────────────────────────────────────────────────── */}
      <section id="preview" className="py-20 sm:py-24 px-4 sm:px-6 max-w-7xl mx-auto space-y-20 sm:space-y-32">
        {SCREENSHOTS.map((s, i) => (
          <div
            key={s.title}
      ...
      {/* ── TESTIMONIALS ─────────────────────────────────────────────────── */}
      <section id="reviews" className="py-20 sm:py-24 px-4 sm:px-6 max-w-7xl mx-auto text-center">
        <h2 className="text-3xl sm:text-4xl font-extrabold mb-12 sm:mb-16">Loved by Users</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-8">
          {REVIEWS.map((r) => (
            <div key={r.name} className="p-7 sm:p-10 rounded-2xl sm:rounded-[2.5rem] bg-white/[0.02] border border-white/5 text-left italic">
              <p className="text-base sm:text-lg text-slate-400 mb-6 sm:mb-8 leading-relaxed">"{r.text}"</p>
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${r.color} flex items-center justify-center font-bold text-white flex-shrink-0`}>
                  {r.avatar}
                </div>
